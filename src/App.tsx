...
import { exportToPdf, copyToClipboard, generateRandomInvoiceNumber } from './utils/exportUtils';
...
const emptyLineItems: LineItem[] = Array(5).fill(null).map(() => ({
  name: '',
  quantity: '',
  unitPrice: '',
  notes: '',
...
function App() {
  ...
  const handleInputChange = useCallback(<K extends keyof InvoiceData>(
    field: K,
    value: InvoiceData[K]
  ) => {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      if (invoiceElement) {
        await exportToPdf(invoiceElement, `invoice-${formData.invoiceNumber.replace(' ', '-')}.pdf`);
        showToast('PDF 已下載');
      }
    } catch (err) {
  ...
  return (
    <Layout
      ...
      preview={
        <div className="relative group w-full h-full flex justify-center" ref={invoiceRef}>
          {/* Toast Notification */}
          {toast && (
